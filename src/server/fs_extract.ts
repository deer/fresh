...
export async function extractRoutes(
  ...
  if (config.dev) {
    const checks: CheckFunction[] = [
      ...
      () => assertStaticDirSafety(config.staticDir ?? "", defaultStaticDir),
      () => assertNoStaticRouteConflicts(routes, staticFiles),
      () => assertPluginsCallRender(config.plugins ?? []),
      () => assertPluginsCallRenderAsync(config.plugins ?? []),
      () => assertPluginsInjectModules(config.plugins ?? []),
    ];
